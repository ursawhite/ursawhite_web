...
const BlogDetails = ({ slug }) => {
  ...
  const getBlogData = async () => {
    ...
    try {
      ...
      const plainText = stripHtmlTags(data[0].acf.blog_content);
      const readingTimeResult = readingTime(plainText, 300);
      setReadingTimeResult(readingTimeResult);
      setBlogData(data);
    } catch (error) {
      console.error(error);
    } finally {
                  ...
                  <div className="blog-author">
                    ...
                    <div>
                      ...
                      <div className="blog-date">
                        {readingTimeResult?.text} · {blog_date}
                      </div>
                    </div>
                  </div>
                  <Image
